...
export function GeneratePanel({ story }: GeneratePanelProps) {
  ...
  const [activeTab, setActiveTab] = useState<'generate' | 'github' | 'parse'>('generate');
  ...
  return (
    <div className="h-full flex flex-col p-6">
      <div className="mb-6">
        <h2 className="text-lg font-semibold">Generate Framework Configs</h2>
        <p className="text-sm text-muted-foreground">
          Generate configuration files for different agent frameworks, or parse existing configs into this story.
        </p>
      </div>
      ...
      >
                ...
                {adapters.map((adapter) => {
                  ...
                  return (
                    <div key={adapter.id} className="border rounded-lg p-3">
                      ...
                      {isExpanded && (
                        <ExpandedAdapterDetails compat={compat} adapter={adapter} />
                      )}
                    </div>
                  );
                })}
        ...
        <TabsContent value="github" className="flex-1 mt-4 overflow-auto">
          <GitHubPublish
            exportResult={exportResult}
            onExportNeeded={() => setActiveTab('generate')}
          />
        </TabsContent>

        {/* Parse Tab */}
        <TabsContent value="parse" className="flex-1 mt-4 overflow-auto">
          <ParseTab />
        </TabsContent>
      </Tabs>
    </div>
...
function ExpandedAdapterDetails({
  compat,
  adapter,
}: {
  compat: HarnessCompatibility | undefined;
  ...
  return (
    <div className="mt-3 ml-7 space-y-2">
      ...
      {compat?.unsupportedFeatures && compat.unsupportedFeatures.length > 0 && (
        <div className="space-y-1">
      ...
      {compat?.missingFeatures && compat.missingFeatures.length > 0 && (
        <div className="space-y-1">
...
function GeneratedFilesPreview({
  ...
  exportResult: HarnessExportResult;
  onDownload: () => void;
}) {
        ...
        <div className="flex gap-4 h-64">
          ...
          <div className="flex-1 border rounded-lg flex flex-col min-w-0">
            <div className="p-2 border-b bg-muted/50 flex items-center justify-between shrink-0">
              <span className="text-xs font-medium truncate">{selectedFile || 'Select a file'}</span>
              {selectedFile && (
...
function ParseTab() {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <h3 className="mb-2 text-lg font-semibold">Parse External Configs</h3>
      <p className="max-w-md text-sm text-muted-foreground mb-6">
        Import agent configurations from Claude, LangGraph, CrewAI, or Letta formats and convert them into an Agent Story.
      </p>
      <Alert className="max-w-md">
        <AlertCircle className="h-4 w-4" />
        <AlertTitle>Coming Soon</AlertTitle>
        <AlertDescription>
          The ability to parse external framework configurations into Agent Stories is planned for a future release.
        </AlertDescription>
      </Alert>
    </div>
  );
